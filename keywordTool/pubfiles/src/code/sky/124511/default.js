...
integration.params = {
  ...
  "plr_URLKeywords": 1,
  'plr_FastInit' : true,
  'plr_PageScanExclude' : ' .site-header, .site-footer, .site-layout-secondary__col2, .ob-widget-section, .OUTBRAIN , .site-nav-desktop__menu-body, .site-nav-mobile'
...
integration.on("adCallResult", function(e) {
	if (e.data.hasSkin) {
		...
		if ($('.site-header__body').length == 1) {
			...
			$("head").append("<style>.advert--banner-wrap{display: none !important;}</style>");
			$('.sdc-site-au').hide();
			$("body").css({
				"overflow" : "visible"
			});
...
integration.on("adServed", function(e) {
	...
	$('.top-nav, .site-nav, #skycomGlobalNav, .site-header').css({
		'max-width' : '1024px',
		'margin' : '0 auto'
	});
