integration.meta = {
	'sectionID' : '125436',
	'siteName' : 'Trusted Reviews - Desktop - (INT exc. UK)',
	'platform' : 'desktop'
};
...
integration.on("adCallResult", function(e) {
	if (e.data.hasSkin) {
		...
		// Adding a custom class provided by the publisher
	}
});

integration.on("adServed", function(e) {
	headerJS.resizeNav();
	// Calling a custom function provided by the publisher to active their functionallity on the navigation
});
...
integration.custom.ScrollEdge = function() {
	...
	$(window).scroll(function() {
		if ($('nav').css("position") === "fixed" && $(window).scrollTop() > 0 && windowSize >= 1459) {
